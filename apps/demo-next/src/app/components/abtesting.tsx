...
function parseId(
  id: string,
): { codec: CodecKey; bitrate: number | null } | null {
...
export default function ABTesting({ results }: Props) {
  const playable = useMemo(
    () => results.filter((r) => r.mime.startsWith("audio/")),
    [results],
  );
  ...
  const available = useMemo(() => {
    const byCodec = new Map<
      CodecKey,
      { bitrates: Set<number>; items: ResultItem[] }
    >();
    for (const r of playable) {
      const p = parseId(r.id);
      if (!p) continue;
      if (!byCodec.has(p.codec))
        byCodec.set(p.codec, { bitrates: new Set<number>(), items: [] });
  ...
  const codecOptions = useMemo(() => {
    ...
    for (const c of CODECS) {
      if (available.has(c.key))
        opts.push({ key: c.key, label: c.label, ext: c.ext });
    }
    return opts;
  }, [available]);
  ...
  useEffect(() => {
    ...
    setSelectedCodec((prev) =>
      prev && available.has(prev) ? prev : codecOptions[0].key,
    );
  ...
  useEffect(() => {
    ...
    if (!entry) return;
    const codecUsesBitrate =
      CODECS.find((c) => c.key === selectedCodec)?.usesBitrate ?? false;
  ...
  function selectedResult(): ResultItem | null {
    ...
    if (!usesBitrate)
      return entry.items.find((r) => parseId(r.id)?.bitrate == null) ?? null;
    if (selectedBitrate == null) return null;
    return (
      entry.items.find((r) => parseId(r.id)?.bitrate === selectedBitrate) ??
      null
    );
  }
  ...
  async function loadBufferFor(
    ...
    const existing = (buffersRef as any)?.current as
  ...
  async function startSelectedAt(offsetSec: number) {
    ...
    const alignedStart = Math.min(
      Math.max(0, baselineOffset + elapsed),
      buf.duration,
    );
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2 text-xs text-zinc-600 dark:text-zinc-400 w-28">
          <span className="tabular-nums">
            {Math.floor(time).toString().padStart(2, "0")}s
          </span>
          <span>/</span>
          <span className="tabular-nums">
            {Math.floor(duration).toString().padStart(2, "0")}s
          </span>
        </div>
